...
function generateMockData(service: Service & { categoryId?: string }) {
  const cat = service.categoryId || 'function-halls'
...
const amenityKeyIcons: Record<string, React.ComponentType<{ size?: number; className?: string }>> = {
  Parking: Car, AC: Snowflake, 'Generator Backup': Sun, 'Dining Hall': UtensilsCrossed,
  'Bridal Room': Heart, WiFi: Wifi, CCTV: Monitor, Stage: Building2,
...
export default function BookingPage() {
  ...
  return (
    <div className="pb-16">
      {/* ── Hero Section ── */}
      <div className="relative w-full h-[50vh] sm:h-[60vh] min-h-[400px] sm:min-h-[500px] mb-8">
        <img src={gallery[0]} alt={service.name} className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-black/10" />

        <button
          onClick={() => navigate(-1)}
          className="absolute top-6 left-6 z-10 flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/10 backdrop-blur-sm text-white text-sm font-semibold hover:bg-white/20 transition-all duration-300"
        >
          <ArrowLeft size={16} />
          Back
        </button>
            ...
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
              <div className="max-w-2xl">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className="px-3 py-1 rounded-full bg-gold/20 text-gold text-[11px] font-semibold uppercase tracking-wide">
                    {categoryNames[service.categoryId || ''] || 'Service'}
                  </span>
                  {service.verified && (
                    <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-white/15 backdrop-blur-sm text-white text-[11px] font-semibold">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-white/80 text-sm">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-4 gap-2 p-4">
              {gallery.slice(0, 4).map((img, i) => (
                ...
                >
                  <img src={img} alt="" className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
